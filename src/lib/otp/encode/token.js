 ...
 * @returns {string}
 */
export function createEncodedOtpToken(credential, expires, otp, resendBlock) {
  return (
    encodeCredential(credential) +
    OTP_SEPARATOR +
    compressNumber(expires) +
    OTP_SEPARATOR +
    otp +
    OTP_SEPARATOR +
    OTP_MAX_ATTEMPTS +
...
export function decodeOtpToken(encodedOtpToken, dateNow = Date.now()) {
  ...
  if (otpToken[OTP]) {
     ...
     */
    if (Number.isNaN(otpToken[ATTEMPTS]) || otpToken[ATTEMPTS] <= 0 || otpToken[ATTEMPTS] > OTP_MAX_ATTEMPTS) {
      return;
    }
...
export function encodeOtpToken(otpToken) {
  let result = encodeCredential(otpToken[CREDENTIAL]) + OTP_SEPARATOR + compressNumber(otpToken[EXPIRES]);

  if (otpToken[OTP]) {
    result += OTP_SEPARATOR + otpToken[OTP] + OTP_SEPARATOR + otpToken[ATTEMPTS];
    if (otpToken[RESEND_BLOCK]) {
      result += OTP_SEPARATOR + compressNumber(otpToken[RESEND_BLOCK]);
      if (otpToken[OTP_BLOCK]) {
        result += OTP_SEPARATOR + compressNumber(otpToken[OTP_BLOCK]);
      }
    } else if (otpToken[OTP_BLOCK]) {
      result += OTP_DOUBLE_SEPARATOR + compressNumber(otpToken[OTP_BLOCK]);
    }
  }
