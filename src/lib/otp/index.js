...
import { ATTEMPTS, OTP, OTP_BLOCK, RESEND_BLOCK } from "#src/lib/otp/encode/token";
...
export async function getOtpTokenList(key, ciphertext, additionalData) {
  try {
    return (await decryptTextSymmetrically(key, ciphertext, additionalData)).split(OTP_TOKEN_SEPARATOR);
  } catch {
...
export function setOtpCookie(cookies, otpData, expires) {
  cookies.set(COOKIE_NAME_OTP, otpData, {
    expires,
    httpOnly: true,
    partitioned: false,
    path: "/",
